...
public class I18n
{
	...
	public static void setLanguage(Locale locale)
	{
		...
		try
		{
			bundles.add(ResourceBundle.getBundle("assets/" + Reference.DOMAIN + "/lang/" + (locale.getLanguage() + "_" + locale.getCountry()).toLowerCase(), locale));
		}
		catch (Exception e)
		{
			ZerraClient.logger().warn("Could not locate language file for \'" + Reference.DOMAIN + "\' locale \'" + locale.getLanguage() + "_" + locale.getCountry() + "\'. Defaulting to english language file.");
			try
			{
				bundles.add(ResourceBundle.getBundle("assets/" + Reference.DOMAIN + "/lang/" + (english.getLanguage() + "_" + english.getCountry()).toLowerCase(), english));
			}
		...
		for (Mod mod : ZerraClient.getInstance().getModManager().loadedMods.values())
		{
			ModInfo info = mod.getModInfo();
			try
			{
				bundles.add(ResourceBundle.getBundle("assets/" + info.getDomain() + "/lang/" + (locale.getLanguage() + "_" + locale.getCountry()).toLowerCase(), locale));
			}
			catch (Exception e)
			{
				try
				{
					bundles.add(ResourceBundle.getBundle("assets/" + info.getDomain() + "/lang/" + (english.getLanguage() + "_" + english.getCountry()).toLowerCase(), english));
				}
				catch (MissingResourceException e1)
				{
				}
