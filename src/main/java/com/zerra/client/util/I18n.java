...
public class I18n {
	...
	public static void setLanguage(Locale locale) {
		...
		// try {
		// bundle = ResourceBundle.getBundle("assets/" + mod.getModName() + "/lang/" + (locale.getLanguage() + "_" + locale.getCountry()).toLowerCase(), locale);
		// } catch (Exception e) {
		// bundle = ResourceBundle.getBundle("assets/" + mod.getModName() + "/lang/" + (english.getLanguage() + "_" + english.getCountry()).toLowerCase(), Locale.ENGLISH);
		// }
		// bundles.add(bundle);
		// }
		...
		try {
			bundle = ResourceBundle.getBundle("assets/" + Zerra.DOMAIN + "/lang/" + (locale.getLanguage() + "_" + locale.getCountry()).toLowerCase(), locale);
		} catch (Exception e) {
			Zerra.logger().warn("Could not locate language file for \'" + Zerra.DOMAIN + "\' locale \'" + locale.getLanguage() + "_" + locale.getCountry() + "\'. Defaulting to english language file.");
			try {
				bundle = ResourceBundle.getBundle("assets/" + Zerra.DOMAIN + "/lang/" + (english.getLanguage() + "_" + english.getCountry()).toLowerCase(), english);
			} catch (MissingResourceException e1) {
				Zerra.logger().warn("Could not locate language file for \'" + Zerra.DOMAIN + "\' locale \'" + english.getLanguage() + "_" + english.getCountry() + "\'");
			}
		}
